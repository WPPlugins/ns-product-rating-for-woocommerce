...
<div class="ns-rating-woocom-post-rate" id="ns-rating-woocom-post-rate-div-<?php echo $post->ID ?>">
    <div id="ns-rating-woocom-rating-container-<?php echo $post->ID ?>" class="ns-rating-woocom-rating">
        ...
        <?php if ( ! empty( $post_rate ) ) : ?>
                <?php _e( 'Rated', 'ns' ) ?>: <b class="ns-rating-woocom-rate ns-rating-woocom-rate<?php echo $post->ID ?>"><?php echo number_format( $post_rate, 2, ',', '.' ) ?></b>
                <?php _e( 'out of', 'ns' ) ?> <b class="ns-rating-woocom-rate-count ns-rating-woocom-rate-count<?php echo $post->ID ?>"><?php echo $rating_count ?></b> <?php echo _n( 'vote', 'votes', $rating_count, 'ns' ) ?>.
        <?php endif; ?>
        <?php if ( empty( $post_rate ) ) : ?>
